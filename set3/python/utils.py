...
bytearray(c1 ^ c2 for c1, c2 in zip(arr1, arr2))
...
def gen_blocks(code, pad=pad_pkcs7, size=16):
    ...
    blocks = [code[i * size: i * size + size] for i in range(num_blocks)]
...
def apply_ECB(mode, input, key, pad=pad_pkcs7):
    ...
    """
    blocks = gen_blocks(input, pad if mode == 'encrypt'and pad  else None)
    aes_f = key.encrypt if mode == 'encrypt' else key.decrypt
    input_str = bytearray().join(blocks])
